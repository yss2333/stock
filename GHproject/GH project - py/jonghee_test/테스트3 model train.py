...
## 1. Load data
df = pd.read_csv('dacon/심화 loaded data/Econ_data.csv')
...
scaler = MinMaxScaler()
scale_cols = ['Adj Close','2-year','5-year','10-year','T10Y2Y','VIXCLS','STLFSI3']
...
feature_cols = ['2-year','5-year','10-year','T10Y2Y','VIXCLS','STLFSI3']
...
hist = model.fit(x_train, y_train, 
          ...
          epochs=100, batch_size=50,        # 100번 학습 - loss가 점점 작아진다, 만약 100번의 학습을 다 하지 않더라도 loss 가 더 줄지 않는다면, 맞춰둔 조건에 따라 조기종료가 이루어진다
          callbacks=[early_stop])

######################################################################################################################################################################################
# 학습 과정에서의 손실값(로스) 기록
# 히스토리 객체에서 train_loss와 val_loss 가져와서 리스트에 저장
